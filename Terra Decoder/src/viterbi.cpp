...
TerraViterbi::TerraViterbi(bool sync_check, float ber_threshold, int insync_after, int outsync_after, int reset_after)
...
//    ENTER synced state
//*****************************************************************************
void TerraViterbi::enter_synced()
{
    d_state = ST_SYNCED;
    d_invalid_packet_count = 0;
...
//
//*****************************************************************************
int TerraViterbi::work(std::vector<std::complex<float>> &in_syms, size_t size, uint8_t *output)
{
...
unsigned char &TerraViterbi::getState()
{
    return d_state;
}